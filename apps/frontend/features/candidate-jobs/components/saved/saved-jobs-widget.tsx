...
import { Building2, Bookmark, X } from "lucide-react";
...
export function SavedJobsWidget({ jobs, onRemove }: SavedJobsWidgetProps) {
  ...
  return (
    <div className="bg-[hsl(var(--card))] border border-[hsl(var(--border))] rounded-xl p-5 shadow-sm space-y-4">
      ...
      <div className="space-y-4">
        {jobs.map(job => (
          <div key={job.id} className="flex gap-3 group relative pr-6">
            ...
            <Button 
              variant="ghost" 
              size="icon" 
              className="absolute right-0 top-0 h-6 w-6 opacity-0 group-hover:opacity-100 transition-opacity text-[hsl(var(--muted-foreground))] hover:text-[hsl(var(--danger))] hover:bg-[hsl(var(--danger)/0.1)]"
